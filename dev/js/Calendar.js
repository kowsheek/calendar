...
(function (root, factory) {
  ...
  function Calendar(settings) {
    ...
    this.optional =       settings.optional || false;
  ...
  Calendar.prototype.calendarSetDates = function() {
    ...
    if (!this.start_date && !this.end_date) {
      ...
      if (old_date.length == 0 && this.optional)
        new_date = '';
  ...
  Calendar.prototype.calendarHTML = function(type) {
    ...
    return this.element.append('<div class="dr-input">' +
      '<div class="dr-dates">' +
        '<div class="dr-date" contenteditable placeholder="'+ this.format.input +'">'+ (this.optional ? '' : moment(this.current_date).format(this.format.input)) +'</div>' +
      '</div>' +
    '</div>' +
